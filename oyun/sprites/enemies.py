...
class Boss(pygame.sprite.Sprite):
    def __init__(self, x, y, projectile_group, warning_group, player_ref, minion_group_ref, difficulty_setting, oyun_ref):
        super().__init__()
        self.image_original = assets.load_image("boss.png", scale=(200,150), smooth=True) # Scale directly
        self.image = self.image_original
        ...
        self.apply_difficulty_settings() # This sets max_hp, speeds, delays, enrage_threshold, etc.
        ...
        self.hit_sound = assets.load_sound("uzayli_vurus.wav") # Boss specific hit?
        self.shoot_sound = assets.load_sound("uzayli_mermi.wav") # Boss specific shoot?
        ...
        self.beam_duration = 1500 # This is the effect duration, not a state flag for beaming
        self.beam_end_time = 0

        self.minion_spawn_timer = pygame.time.get_ticks()
        self.barrage_end_time = 0
        self.last_barrage_shot = 0
        self.hit_flash_timer = 0 # Renamed from flash_timer for consistency
        self.hit_flash_duration = 100
    ...
    def update(self):
        ...
        if self.hit_flash_timer > 0:
            now = pygame.time.get_ticks()
            if now - self.hit_flash_timer < self.hit_flash_duration:
                if (now // (self.hit_flash_duration // 4)) % 2 == 0: # Fast flicker
                    flash_image = self.image_original.copy()
                    flash_image.fill((255,255,255,120), special_flags=pygame.BLEND_RGBA_MULT) # Semi-transparent white
                    self.image = flash_image
                else:
                    self.image = self.image_original
            else:
            ...
            self.image = self.image_original # Ensure image is original if not flashing
    ...
    def _prepare_attack(self):
        ...
        else: self.attack_target_pos = None # For spread or barrage where warning isn't positional sprite
    ...
    def _execute_attack(self):
        s_spd, t_spd = (9,12) # Spread, Targeted speeds
    ...
    def _spawn_minions(self):
        ...
        for i in range(num_minions):
            minion = Minion(start_x + i * spacing, self.rect.bottom + 10, self.speed * 0.8, self, 
                            self.minion_group, self.oyun_ref.uzayli_mermi_grup, 
                            self.oyun_ref.uzayli_mermi_sesi, self.oyun_ref.bolum_no, self.difficulty)
    ...
    def take_damage(self, amount):
        if not self.is_moving: return # Don't take damage if already defeated
        self.current_hp -= amount
        self.got_hit() # Trigger flash
        ...
        if self.current_hp <= 0:
            ...
            if self.oyun_ref: 
                ...
                # self.kill() # Let cutscene handle visual disappearance

    def got_hit(self):
        self.hit_flash_timer = pygame.time.get_ticks()

    def draw_health_bar(self, surface):
        bar_w = settings.GENISLIK*0.6; bar_h = 25; bar_x = (settings.GENISLIK-bar_w)//2; bar_y = 15
        ...
        if "minik" in assets.FONTS: 
            assets.FONTS["minik"].render_to(surface, (bar_x - 60, bar_y + bar_h // 2 - assets.FONTS["minik"].get_height()//2), "BOSS", settings.BEYAZ)


    # draw() method for Boss is implicitly handled by sprite group draw,
    # but if custom drawing logic beyond hit flash is needed, it can be re-added.
    # The hit flash is now handled in update().
...
class Minion(pygame.sprite.Sprite):
    def __init__(self, x, y, speed, boss_ref, minion_group_ref, mermi_grup, mermi_sesi, level_no, difficulty_setting):
        ...
        self.image = self.image_original
        if self.image.get_width() != 40: # Placeholder check
        ...
        self.max_bullets_on_screen_from_this_minion = 1 # Minions shoot one at a time.
    ...
    def update(self):
        ...
        if self.boss.alive() and self.boss.is_moving:
            ...
            self.rect.y += self.speed # Fall if boss dead/stopped
            if self.rect.top > settings.YUKSEKLIK: self.kill()

        now = pygame.time.get_ticks()
        # Count bullets shot specifically by this minion instance if needed, or use a simpler global count.
        # For now, let's assume a simpler global count for minion bullets from uzayliMermi.
        current_total_minion_or_alien_bullets = sum(1 for b in self.mermi_grup.sprites() if isinstance(b, uzayliMermi))
        
        # Allow minions to fire more freely, maybe cap total alien bullets instead of per-minion.
        # The original logic was "max_bullets_per_minion * len(self.minion_group)"
        # For now, let's have a simpler cap based on total alien bullets on screen from Oyun if needed.
        # The uzayli class uses "self.shoot_max_bullets" (a global cap for aliens)
        # Minions are simpler, let them shoot if timer is up and boss is active.
        max_allowed_enemy_bullets = self.boss.oyun_ref.max_enemy_bullets_on_screen if hasattr(self.boss.oyun_ref, 'max_enemy_bullets_on_screen') else 10 # Fallback

        if self.boss.alive() and self.boss.is_moving and \
           now - self.last_shot_time > self.fire_rate and \
           current_total_minion_or_alien_bullets < max_allowed_enemy_bullets:
            self.ates()
